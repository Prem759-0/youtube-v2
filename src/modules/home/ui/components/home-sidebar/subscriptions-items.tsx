						...
						subscriptions?.pages
							...
							.map((subscription) => (
								<SidebarMenuItem key={`${subscription.creatorId}-${subscription.viewerId}`}>
									...
									>
										<Link prefetch prefetch href={`/users/${subscription.user.id}`} className='flex items-center gap-4'>
					...
					{!isLoading && (
						<SidebarMenuItem>
							<SidebarMenuButton isActive={pathname === '/subscriptions'} asChild>
								<Link prefetch prefetch href='/subscriptions' className='flex items-center gap-4'>
